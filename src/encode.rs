...
pub struct SinglePermutationEncoded<'a, 'b> {
    s: &'a [u8],
    key: &'b [usize],
}

impl<'a, 'b> SinglePermutationEncoded<'a, 'b> {
    pub fn new(s: &'a [u8], key: &'b [usize]) -> Result<Self, EncoderCreationError> {
        ...
        if q != key_entries { return Err(EncoderCreationError::BadKey) }

        Ok(SinglePermutationEncoded { s, key })
    }
    ...
    pub fn get(&self, index: usize) -> Option<u8> {
        ...
        let i_in_decoded = self.key.iter().position(|x| *x == j).unwrap();
        let j_in_decoded = i;
